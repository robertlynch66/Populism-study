...
mod5 <- data %>% select(sanders_perc_16,drug_16_over_08,alcohol_16_over_08,pop_density_2014,
                        ...
                        sk2014_over_2005,percent_white_16_to_8, pop_change_16_to_8,             
                        percent_hispanic_16_to_8,median_hh_income_16_to_8,pop_2014,
                        suicides_16_scaled,alcohol_16_scaled,drug_16_scaled,
                        soc_cap_indx_14,perc_white,perc_hisp,median_hh_income)      
...
## Rethinking Bayes code for model 5 #################################
##################################################################
# Model 5 sanders vs clinton longitudnal
library(dplyr)
library(rethinking)
# path to the folder with the R data files
path<- (paste0("~/r_files/"))
...
p <- p %>% select(state_id,sanders_primary, sk2014_over_2005, suicides_16_over_08, drug_16_over_08,
                  alcohol_16_over_08,
                  ...
                  pop_2014,total_votes_16_primary)
...
path<- (paste0("results/"))
filename <- "sanders_vs_clinton_long_model.rds"

saveRDS(model, paste0(path, filename))

## Rethinking Bayes code for model 6 #################################
##################################################################
# Model 6 trump vs vs clinton long
library(dplyr)
library(rethinking)
# path to the folder with the R data files
path<- (paste0("~/r_files/"))
...
path<- (paste0("results/"))
filename <- "trump_vs_clinton_long_model.rds"
