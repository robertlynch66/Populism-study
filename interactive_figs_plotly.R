...
p$trump_percent_general <- p$trump_16/(p$trump_16+p$clinton_16)
p$sanders_percent <- p$sanders_primary1/(p$sanders_primary1+p$clinton_primary1)
p$trump_percent_primaryk <- p$trump_primary_votes/(p$trump_primary_votes+p$kasich_votes)

# populist counties - N=5
pop <- filter (p, trump_percent_general>0.7, sanders_percent>0.45,
                       trump_percent_primaryk>0.85, suicides_16_to_8>10, alcohol_16_to_8 >10, drugs_16_to_8>10,sk2014_over_2005>0.1)

# N== 1

nonpop <- filter (p, trump_percent_general<0.6, sanders_percent<0.4,
               trump_percent_primaryk<0.75, suicides_16_to_8>0, alcohol_16_to_8 <0, drugs_16_to_8<0,sk2014_over_2005<0)

# make main figure interactive with plotly
...
#subset predicted libne (shorten it)
pred_df2 <- pred_df2[c(8:30,57:68,85:110,142:155), ]
# full interactive
# change label names
pred_df2$county <- 0
pred_df2$state <- 0
p4$mortality_change = p4$new_mort
pred_df2$mortality_change = pred_df2$new_mort
p4$population_in_2016 = p4$pop_16
p4$category = p4$mort_cat
pred_df2$category = pred_df2$mort_cat



p1 <- ggplot(p4, aes(label = county, label2=state)) + 
  
  geom_point(aes(size = population_in_2016,x=mortality_change,y=predicted_trump_percent, colour=category), alpha=0.7)+
  ...
  facet_grid(.~category, labeller=labeller(category=labels),scales="free", shrink=TRUE)+
  
  #### Fix the geom_point y axis point
  geom_ribbon(data=pred_df2, mapping=aes(ymin=lower, ymax=upper, x=mortality_change,
                                         colour=category, fill = category,alpha = 2),show.legend=FALSE)+
  ...
  scale_y_continuous(name="Percentage of votes for Trump vs Clinton",
                     breaks=c(0.25,0.50,0.75),limits=c(0,1),
                     labels=c('25%','50%','75%'))+

  
  scale_x_continuous(name="Change in deaths per 100k and\nthe social capital index",
                     limits=c(-100,100), breaks=c(-40,-20,0,20,40),labels=c('Q1','Q2','Q3','Q4','Q5'))+
  ...
  theme(legend.text = element_text(size = 7, face = "bold"),
        ...
        plot.margin=unit(c(1.4,1.3,1,1),"cm"),
...
###### make facet plots interactive
library(plotly)

w <-ggplotly(p1)
# save the html file
htmlwidgets::saveWidget(as_widget(w), "fig_1.html")
...
p2 <- ggplot(p4, aes(label = county, label2=state)) + 
  
  geom_point(aes(size = population_in_2016,x=mortality_change,y=predicted_sanders_percent, colour=category), alpha=0.7)+
  ...
  scale_color_hue(labels = c("Alcohol", "Drugs","Suicides","Social capital"))+
  scale_y_continuous(name="Percentage of votes for Sanders vs Clinton",
                     breaks=c(0.25,0.50,0.75),limits=c(0,1),
                     labels=c('25%','50%','75%'))+
...
w <-ggplotly(p2)
# save the html file
htmlwidgets::saveWidget(as_widget(w), "fig_2.html")
...
p3 <- ggplot(p4, aes(label = county, label2=state)) + 
  ...
  scale_color_hue(labels = c("Alcohol", "Drugs","Suicides","Social capital"))+
  scale_y_continuous(name="Percentage of votes for Trump vs Kasich",
                     breaks=c(0.25,0.50,0.75),limits=c(0,1),
                     labels=c('25%','50%','75%'))+
...
w <-ggplotly(p3)
# save the html file
htmlwidgets::saveWidget(as_widget(w), "fig_3.html")


### ??????????Make county map interactive??????????????????
###############################
###############################
###############################
###############################
...
Data <- readRDS("../data files/populism_data_w_geom.rds")
#### get rid of some variables
# make populist counties variable (2=populist, 1=traditonal 0= neutral)
Data$County  <- ifelse(Data$trump_16 > Data$clinton_16 &  Data$sanders_primary1 > Data$clinton_primary1 & Data$obama_08 >Data$mccain_08 , "Populist",
                     ifelse(Data$trump_16 < Data$clinton_16 & Data$sanders_primary1 < Data$clinton_primary1 ,"Traditional","Neutral"))
Data$County[is.na(Data$County)] <- "Neutral"
...
# get rid of alaska
Data<- Data[c(1:3121,3151:3155),]


Data <- Data %>% select (1,2,11,12,13,14,63,64,65,66,68,69,70,71,72,73,74,76,89,90,91)
Data$population_in_2016 <- Data$pop_16
Data$pop_16<- NULL
#plot(p["suicides_16_to_8"])
library(mapview)
## try to save this interactive map

d <- mapview(Data, zcol = "County", legend = TRUE,easter.egg=TRUE, label=Data$NAME,na.color="black",
             col.regions = mapviewGetOption("raster.palette")(256))
d

## Save the image html and the asscoate leaflet files


mapshot(d, url=("../../Interactive map files/county_map.html"))

#####
# main map linked to at end of results section
# read in my data
library(dplyr)
library(ggplot2)
# read in the geom file
...
Data$Trump_percent_general <- Data$trump_16/(Data$trump_16+Data$clinton_16)
Data$Sanders_percent_primary <- Data$sanders_primary1/(Data$sanders_primary1+Data$clinton_primary1)
Data$kasich_votes <- as.numeric(Data$kasich_votes)
Data$Trump_percent_primary <- Data$trump_primary_votes/(Data$trump_primary_votes+Data$kasich_votes)
Data$Social_capital_change <-Data$sk2014_over_2005
Data$Population_change<-Data$pop_change_16_to_10      
Data$Median_household_income_change <- Data$median_hh_income_16_to_10
Data$Percent_White_change<- Data$white_16_to_10
Data$Percent_w_bachelors_change<-Data$bachelors_16_to_10       
Data$Percent_male_unemployment_change <- Data$male_unemplmt_16_to_10   
Data$Percent_female_unemployment_change <- Data$female_unemplmt_16_to_10
Data$Percent_foreign_born_change <- Data$for_born_16_to_10
Data$Per_capita_change_in_alcohol_deaths<-Data$alcohol_16_to_8          
Data$Per_capita_change_in_drug_deaths <- Data$drugs_16_to_8
Data$Per_capita_change_in_suicides<-Data$suicides_16_to_8
Data$Population_in_2016 <- Data$pop_16
Data$County  <- ifelse(Data$trump_16 > Data$clinton_16 &  Data$sanders_primary1 > Data$clinton_primary1 & Data$obama_08 >Data$mccain_08 , "Populist",
                       ifelse(Data$trump_16 < Data$clinton_16 & Data$sanders_primary1 < Data$clinton_primary1 ,"Traditional","Neutral"))
Data$County[is.na(Data$County)] <- "Neutral"
Data <- Data %>% select (1,2,91:106)
...
z <- mapview(Data, zcol = "County", legend = TRUE,easter.egg=TRUE, label=Data$NAME,na.color="black",
             col.regions = mapviewGetOption("raster.palette")(256))
z
...
mapshot(z, url=("../../Interactive map files/main_county_map.html"))
