...
ed_groups <- education %>%
  ...
  mutate(edgroup = case_when(
    variable < "B15003_016" ~ "did not finish HS", 
    variable < "B15003_022" ~ "high school to bachelors", 
    variable < "B15003_023" ~ "bachelors", 
    variable < "B15003_024" ~ "masters", 
    variable < "B15003_025" ~ "professional",
    TRUE  ~ "doctorate")) %>%
...
dot_plot <- map(c("did not finish HS", "high school to bachelors", "bachelors", "masters",
                  "professional",
"doctorate"), function(group) {
  z %>%
    filter(edgroup == group) %>%
    st_sample(., size = .$group_est / 100) %>%
    st_sf() %>%
    mutate(group = group) 
}) %>%
  reduce(rbind) %>%
...
plot <- ggplot() + 
  ...
  scale_color_manual("", 
                    breaks = c("did not finish HS", "high school to bachelors",
